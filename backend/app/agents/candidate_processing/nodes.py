...
async def load_candidate_and_job(state: CandidateProcessingState) -> dict:
    ...
    if (
        ...
        existing_score = await CandidateRepository(
            state["session"]
        ).latest_score(candidate.id, job.id)
        if existing_score:
            return {
...
async def extract_resume_text(state: CandidateProcessingState) -> dict:
    ...
    if state.get("resume_text"):
        return {}
    return {
        "resume_text": extract_text_from_file(candidate.resume_file_path or "")
    }
...
async def extract_links(state: CandidateProcessingState) -> dict:
    ...
    links.update(
        match.group(0)
        for match in LINK_RE.finditer(state.get("resume_text", ""))
    )
...
async def apply_basic_rules(state: CandidateProcessingState) -> dict:
    ...
    if (
        score.get("overall_score", 0) < 50
        and score.get("recommendation") == "POSSIBLE_MATCH"
    ):
...
async def persist_candidate_results(state: CandidateProcessingState) -> dict:
    ...
    next_status = (
        ...
        if state["candidate_score"]["recommendation"] == "NEEDS_REVIEW"
